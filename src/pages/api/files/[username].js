import {getDownloadUrl, getUploadUrl} from "@lib/minioClient";

export default async function handler(req, res) {

    if (req.method === "POST") {
        return fetchFiles(req, res)
    ...
    try {
        const urls = await Promise.all(
            files.map(async ({projectId}) => {
                ...
                const putUrl = await getUploadUrl(bucket, path, 60 * 5); // 5 min expiry
...
async function fetchFiles(req, res) {
    ...
    try {
        const urls = await Promise.all(
            files.map(async ({projectId, file}) => {
                const path = `${username}/${file}`;
                const getUrl = await getDownloadUrl(bucket, path, 60 * 5); // 5 min expiry
                //const getUrl = await minioClient.presignedGetObject(bucket, path, 60 * 60 * 24); // 24h expiry
